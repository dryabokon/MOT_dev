...
class Tracker_boxmot:
    def __init__(self,folder_out,device='cuda:0',algorithm='DEEPSORT'):
        ...
        # !fails
        # self.tracker = StrongSORT(model_weights=Path('osnet_x0_25_msmt17.pt'),device=device,fp16=False,per_class=True)
...
# ----------------------------------------------------------------------------------------------------------------------
    def track_detections(self,df_det,filename_in,frame_id=None,do_debug=False):
        ...
        confs = numpy.array([a.conf for a in self.tracker.active_tracks if a.history_observations and (len(a.history_observations) > 2)])
